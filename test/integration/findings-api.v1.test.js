...
describe('FindingsApiV1 service setup', () => {
  ...
  test('should fail while setting wrong url', async () => {
    ...
    try {
      ...
      await client.setServiceUrl('https://uk-south.secadvisor.cloud.ibm.com/findings');
      throw new Error('Should not pass');
    } catch (err) {
      expect(err.message).toEqual(
        'The service URL you specified is incorrect for the location selected for the account. The correct URL is: https://us-south.secadvisor.cloud.ibm.com/findings'
      );
    }
  });
  ...
  test('should use setup service using newInstance', async () => {
    jest.setTimeout(30000);
    const client = await FindingsApiV1.newInstance({
      authenticator: new IamAuthenticator({ apikey: process.env.apikey }),
      serviceUrl: process.env.findingsServiceUrl,
    });
  ...
  test('should use setup service using newInstance and authenticator from env', async () => {
    ...
    process.env.FINDINGS_API_APIKEY = process.env.apikey;
    const client = await FindingsApiV1.newInstance({
      authenticator: new IamAuthenticator({ apikey: process.env.apikey }),
      serviceUrl: process.env.findingsServiceUrl,
    });
  ...
  test('should use setup service using newInstance and fail while setting up with wrong url', async () => {
    ...
    try {
      await FindingsApiV1.newInstance({
        authenticator: new IamAuthenticator({ apikey: process.env.apikey }),
        serviceUrl: 'https://uk-south.secadvisor.cloud.ibm.com/findings',
      });
      throw new Error('Should not pass');
    } catch (err) {
      expect(err.message).toEqual(
  ...
  test('should use setup service using newInstance and fail with no authenticator', async () => {
    jest.setTimeout(30000);
    delete process.env.FINDINGS_API_AUTH_TYPE;
    delete process.env.FINDINGS_API_APIKEY;
    try {
      await FindingsApiV1.newInstance({
        serviceUrl: process.env.findingsServiceUrl,
      });
